...
- Updated the Admin UI to use the latest JS SDK to resolve the `isNew` record field conflict ([#2385](https://github.com/alvarolm/pocketbase/discussions/2385)).

- Fixed `editor` field fullscreen `z-index` ([#2410](https://github.com/alvarolm/pocketbase/issues/2410)).

- Inserts the default app settings as part of the system init migration so that they are always available when accessed from within a user defined migration ([#2423](https://github.com/alvarolm/pocketbase/discussions/2423)).


## v0.15.2

- Fixed View query `SELECT DISTINCT` identifiers parsing ([#2349-5706019](https://github.com/alvarolm/pocketbase/discussions/2349#discussioncomment-5706019)).

- Fixed View collection schema incorrectly resolving multiple aliased fields originating from the same field source ([#2349-5707675](https://github.com/alvarolm/pocketbase/discussions/2349#discussioncomment-5707675)).
...
- Trigger the related `Record` model realtime subscription events on [custom model struct](https://pocketbase.io/docs/custom-models/) save ([#2325](https://github.com/alvarolm/pocketbase/discussions/2325)).
...
- Enabled RTL for the TinyMCE editor ([#2327](https://github.com/alvarolm/pocketbase/issues/2327)).
...
- Simplified the OAuth2 authentication flow in a single "all in one" call ([#55](https://github.com/alvarolm/pocketbase/issues/55)).
...
- Added support for protected files ([#215](https://github.com/alvarolm/pocketbase/issues/215)).
  Requires JS SDK v0.14.0+ or Dart SDK v0.9.0+.
...
- Added checks for `nil` hooks in `forms.RecordUpsert` when used with custom `Dao` ([#2277](https://github.com/alvarolm/pocketbase/issues/2277)).

- Fixed unique detailed field error not returned on record create failure ([#2287](https://github.com/alvarolm/pocketbase/discussions/2287)).


## v0.14.4

- Fixed concurrent map write pannic on `list.ExistInSliceWithRegex()` cache ([#2272](https://github.com/alvarolm/pocketbase/issues/2272)).


## v0.14.3

- Fixed Admin UI Logs `meta` visualization in Firefox ([#2221](https://github.com/alvarolm/pocketbase/issues/2221)).

- Downgraded to v1 of the `aws/aws-sdk-go` package since v2 has compatibility issues with GCS ([#2231](https://github.com/alvarolm/pocketbase/issues/2231)).
...
- Updated the underlying S3 lib to use `aws-sdk-go-v2` ([#1346](https://github.com/alvarolm/pocketbase/pull/1346); thanks @yuxiang-gao).
...
- Added support for advanced unique constraints and indexes management ([#345](https://github.com/alvarolm/pocketbase/issues/345), [#544](https://github.com/alvarolm/pocketbase/issues/544))
...
- Removed the `COALESCE` wrapping from some of the generated filter conditions to make better use of the indexes ([#1939](https://github.com/alvarolm/pocketbase/issues/1939)).

- Detect `id` aliased view columns as single `relation` fields ([#2029](https://github.com/alvarolm/pocketbase/discussions/2029)).
...
- Simplified the default mail template styles to allow more control over the template layout ([#1904](https://github.com/alvarolm/pocketbase/issues/1904)).

- Added option to explicitly set the record id from the Admin UI ([#2118](https://github.com/alvarolm/pocketbase/issues/2118)).
...
- ⚠️ Registered the `RemoveTrailingSlash` middleware only for the `/api/*` routes since it is causing issues with subpath file serving endpoints ([#2072](https://github.com/alvarolm/pocketbase/issues/2072)).

- ⚠️ Changed the request logs `method` value to UPPERCASE, eg. "get" => "GET" ([#1956](https://github.com/alvarolm/pocketbase/discussions/1956)).
...
- Fixed view collections import ([#2044](https://github.com/alvarolm/pocketbase/issues/2044)).
...
- Fixed Admin UI js error when selecting multiple `file` field as `relation` "Display fields" ([#1989](https://github.com/alvarolm/pocketbase/issues/1989)).


## v0.13.1

- Added `HEAD` request method support for the `/api/files/:collection/:recordId/:filename` route ([#1976](https://github.com/alvarolm/pocketbase/discussions/1976)).
...
- Added auto fail/retry (default to 8 attempts) for the `SELECT` queries to gracefully handle the `database is locked` errors ([#1795](https://github.com/alvarolm/pocketbase/discussions/1795#discussioncomment-4882169)).
...
- Fixed "Toggle column" reactivity when navigating between collections ([#1836](https://github.com/alvarolm/pocketbase/pull/1836)).

- Logged the current datetime on server start ([#1822](https://github.com/alvarolm/pocketbase/issues/1822)).


## v0.12.2

- Fixed the "Clear" button of the datepicker component not clearing the value ([#1730](https://github.com/alvarolm/pocketbase/discussions/1730)).

- Increased slightly the fields contrast ([#1742](https://github.com/alvarolm/pocketbase/issues/1742)).
...
- Fixed `overlay-active` css class not being removed on nested overlay panel close ([#1718](https://github.com/alvarolm/pocketbase/issues/1718)).

- Added the collection name in the page title ([#1711](https://github.com/alvarolm/pocketbase/issues/1711)).


## v0.12.0

- Refactored the relation picker UI to allow server-side search, sort, create, update and delete of relation records ([#976](https://github.com/alvarolm/pocketbase/issues/976)).

- Added new `RelationOptions.DisplayFields` option to specify custom relation field(s) visualization in the Admin UI.

- Added Authentik OAuth2 provider ([#1377](https://github.com/alvarolm/pocketbase/pull/1377); thanks @pr0ton11).

- Added LiveChat OAuth2 provider ([#1573](https://github.com/alvarolm/pocketbase/pull/1573); thanks @mariosant).

- Added Gitea OAuth2 provider ([#1643](https://github.com/alvarolm/pocketbase/pull/1643); thanks @hlanderdev).

- Added PDF file previews ([#1548](https://github.com/alvarolm/pocketbase/pull/1548); thanks @mjadobson).

- Added video and audio file previews.

- Added rich text editor (`editor`) field for HTML content based on TinyMCE ([#370](https://github.com/alvarolm/pocketbase/issues/370)).
...
- Added "Duplicate" Collection and Record options in the Admin UI ([#1656](https://github.com/alvarolm/pocketbase/issues/1656)).

- Added `filesystem.GetFile()` helper to read files through the FileSystem abstraction ([#1578](https://github.com/alvarolm/pocketbase/pull/1578); thanks @avarabyeu).
...
- Added new helper `apis.RecordAuthResponse(app, httpContext, record, meta)` to return a standard Record auth API response ([#1623](https://github.com/alvarolm/pocketbase/issues/1623)).
...
- ⚠️ Fixed the `json` field **string** value normalization ([#1703](https://github.com/alvarolm/pocketbase/issues/1703)).
...
- Added `aria-label` to some of the buttons in the Admin UI for better accessibility ([#1702](https://github.com/alvarolm/pocketbase/pull/1702); thanks @ndarilek).

- Updated the filename extension checks in the Admin UI to be case-insensitive ([#1707](https://github.com/alvarolm/pocketbase/pull/1707); thanks @hungcrush).
...
- Fixed cascade delete for rel records with the same id as the main record ([#1689](https://github.com/alvarolm/pocketbase/issues/1689)).


## v0.11.3

- Fix realtime API panic on concurrent clients iteration ([#1628](https://github.com/alvarolm/pocketbase/issues/1628))
...
- Fixed `fs.DeleteByPrefix()` hang on invalid S3 settings ([#1575](https://github.com/alvarolm/pocketbase/discussions/1575#discussioncomment-4661089)).
...
- Unescaped path parameter values ([#1552](https://github.com/alvarolm/pocketbase/issues/1552)).
...
- Added Strava OAuth2 provider ([#1443](https://github.com/alvarolm/pocketbase/pull/1443); thanks @szsascha).

- Added Gitee OAuth2 provider ([#1448](https://github.com/alvarolm/pocketbase/pull/1448); thanks @yuxiang-gao).

- Added IME status check to the textarea keydown handler ([#1370](https://github.com/alvarolm/pocketbase/pull/1370); thanks @tenthree).

- Added `filesystem.NewFileFromBytes()` helper ([#1420](https://github.com/alvarolm/pocketbase/pull/1420); thanks @dschissler).

- Added support for reordering uploaded multiple files.

- Added `webp` to the default images mime type presets list ([#1469](https://github.com/alvarolm/pocketbase/pull/1469); thanks @khairulhaaziq).

- Added the OAuth2 refresh token to the auth meta response ([#1487](https://github.com/alvarolm/pocketbase/issues/1487)).

- Fixed the text wrapping in the Admin UI listing searchbar ([#1416](https://github.com/alvarolm/pocketbase/issues/1416)).

- Fixed number field value output in the records listing ([#1447](https://github.com/alvarolm/pocketbase/issues/1447)).

- Fixed duplicated settings view pages caused by uncompleted transitions ([#1498](https://github.com/alvarolm/pocketbase/issues/1498)).

- Allowed sending `Authorization` header with the `/auth-with-password` record and admin login requests ([#1494](https://github.com/alvarolm/pocketbase/discussions/1494)).
...
- Fixed `Record.MergeExpand` panic when the main model expand map is not initialized ([#1365](https://github.com/alvarolm/pocketbase/issues/1365)).


## v0.10.3

- ⚠️ Renamed the metadata key `original_filename` to `original-filename` due to an S3 file upload error caused by the underscore character ([#1343](https://github.com/alvarolm/pocketbase/pull/1343); thanks @yuxiang-gao).

- Fixed request verification docs api url ([#1332](https://github.com/alvarolm/pocketbase/pull/1332); thanks @JoyMajumdar2001)

- Excluded `collectionId` and `collectionName` from the displayable relation props list ([1322](https://github.com/alvarolm/pocketbase/issues/1322); thanks @dhall2).


## v0.10.2

- Fixed nested multiple expands with shared path ([#586](https://github.com/alvarolm/pocketbase/issues/586#issuecomment-1357784227)).
...
- Fixed nested transactions deadlock when authenticating with OAuth2 ([#1291](https://github.com/alvarolm/pocketbase/issues/1291)).
...
- Added support for SMTP `LOGIN` auth for Microsoft/Outlook and other providers that don't support the `PLAIN` auth method ([#1217](https://github.com/alvarolm/pocketbase/discussions/1217#discussioncomment-4387970)).
...
- Fixed `~` expressions backslash literal escaping ([#1231](https://github.com/alvarolm/pocketbase/discussions/1231)).

- Refactored the `core.app.Bootstrap()` to be called before starting the cobra commands ([#1267](https://github.com/alvarolm/pocketbase/discussions/1267)).
...
- Fixed field column name conflict on record deletion ([#1220](https://github.com/alvarolm/pocketbase/discussions/1220)).
...
- Added `Dao` query semaphore and base fail/retry handling to improve the concurrent writes throughput ([#1187](https://github.com/alvarolm/pocketbase/issues/1187)).

- Fixed records cascade deletion when there are "A<->B" relation references.

- Replaced `c.QueryString()` with `c.QueryParams().Encode()` to allow loading middleware modified query parameters in the default crud actions ([#1210](https://github.com/alvarolm/pocketbase/discussions/1210)).

- Fixed the datetime field not triggering the `onChange` event on manual field edit and added a "Clear" button ([#1219](https://github.com/alvarolm/pocketbase/issues/1219)).
...
- Fixed concurrent multi-relation cascade update/delete ([#1138](https://github.com/alvarolm/pocketbase/issues/1138)).

- Added the raw OAuth2 user data (`meta.rawUser`) and OAuth2 access token (`meta.accessToken`) to the auth response ([#654](https://github.com/alvarolm/pocketbase/discussions/654)).
...
- Added support for `Partial/Range` file requests ([#1125](https://github.com/alvarolm/pocketbase/issues/1125)).
...
The biggest change is the merge of the `User` models and the `profiles` collection per [#376](https://github.com/alvarolm/pocketbase/issues/376).
...
- Added indirect expand support [#312](https://github.com/alvarolm/pocketbase/issues/312#issuecomment-1242893496).

- The `json` field type now supports filtering and sorting [#423](https://github.com/alvarolm/pocketbase/issues/423#issuecomment-1258302125).
...
- Improved memory usage on large file uploads [#835](https://github.com/alvarolm/pocketbase/discussions/835).
...
- Moved the formatted `ApiError` struct and factories to the `github.com/alvarolm/pocketbase/apis` subpackage:
  <table class="d-table" width="100%">
    <tr>
      <th>Old</th>
      <th>New</th>
    ...
    <tr valign="top">
      <td>github.com/alvarolm/pocketbase/<strong>tools/rest</strong></td>
      <td>github.com/alvarolm/pocketbase/<strong>apis</strong></td>
    </tr>
    <tr valign="top">
