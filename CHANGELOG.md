...
- Added _partial_ WebP thumbs generation (_the thumbs will be stored as PNG_; [#6744](https://github.com/alvarolm/pocketbase/pull/6744)).
...
- ⚠️ Moved the Create and Manage API rule checks out of the `OnRecordCreateRequest` hook finalizer, **aka. now all CRUD API rules are checked BEFORE triggering their corresponding `*Request` hook**.
    ...
    ```
    If you are having difficulties adjusting your code, feel free to open a [Q&A discussion](https://github.com/alvarolm/pocketbase/discussions) with the failing/problematic code sample.
...
- Updated the `select` field UI to accommodate better larger lists and RTL languages ([#4674](https://github.com/alvarolm/pocketbase/issues/4674)).
...
- Forced `text/javascript` Content-Type when serving `.js`/`.mjs` collection uploaded files with the `/api/files/...` endpoint ([#6597](https://github.com/alvarolm/pocketbase/issues/6597)).

- Added second optional JSVM `DateTime` constructor argument for specifying a default timezone as TZ identifier when parsing the date string as alternative to a fixed offset in order to better handle daylight saving time nuances ([#6688](https://github.com/alvarolm/pocketbase/discussions/6688)):
    ```js
...
- Allow OIDC `email_verified` to be int or boolean string since some OIDC providers like AWS Cognito has non-standard userinfo response ([#6657](https://github.com/alvarolm/pocketbase/pull/6657)).
...
- Fixed canonical URI parts escaping when generating the S3 request signature ([#6654](https://github.com/alvarolm/pocketbase/issues/6654)).


## v0.26.4

- Fixed `RecordErrorEvent.Error` and `CollectionErrorEvent.Error` sync with `ModelErrorEvent.Error` ([#6639](https://github.com/alvarolm/pocketbase/issues/6639)).
...
- Fixed and normalized logs error serialization across common types for more consistent logs error output ([#6631](https://github.com/alvarolm/pocketbase/issues/6631)).
...
- Removed the wrapping of `io.EOF` error when reading files since currently `io.ReadAll` doesn't check for wrapped errors ([#6600](https://github.com/alvarolm/pocketbase/issues/6600)).


## v0.26.0

- ⚠️ Replaced `aws-sdk-go-v2` and `gocloud.dev/blob` with custom lighter implementation ([#6562](https://github.com/alvarolm/pocketbase/discussions/6562)).
...
- Load the request info context during password/OAuth2/OTP authentication ([#6402](https://github.com/alvarolm/pocketbase/issues/6402)).
...
- Added `$os.stat(file)` JSVM helper ([#6407](https://github.com/alvarolm/pocketbase/discussions/6407)).

- Added log warning for `async` marked JSVM handlers and resolve when possible the returned `Promise` as fallback ([#6476](https://github.com/alvarolm/pocketbase/issues/6476)).

- Allowed calling `cronAdd`, `cronRemove` from inside other JSVM handlers ([#6481](https://github.com/alvarolm/pocketbase/discussions/6481)).
...
- Bumped the default request read and write timeouts to 5mins (_old 3mins_) to accommodate slower internet connections and larger file uploads/downloads.
    _If you want to change them you can modify the `OnServe` hook's `ServeEvent.ReadTimeout/WriteTimeout` fields as shown in [#6550](https://github.com/alvarolm/pocketbase/discussions/6550#discussioncomment-12364515)._

- Normalized the `@request.auth.*` and `@request.body.*` back relations resolver to always return `null` when the relation field is pointing to a different collection ([#6590](https://github.com/alvarolm/pocketbase/discussions/6590#discussioncomment-12496581)).
...
- Fixed `DynamicModel` object/array props reflect type caching ([#6563](https://github.com/alvarolm/pocketbase/discussions/6563)).


## v0.25.8

- Added a default leeway of 5 minutes for the Apple/OIDC `id_token` timestamp claims check to account for clock-skew ([#6529](https://github.com/alvarolm/pocketbase/issues/6529)).
...
- Fixed `@request.body.jsonObjOrArr.*` values extraction ([#6493](https://github.com/alvarolm/pocketbase/discussions/6493)).


## v0.25.6

- Restore the missing `meta.isNew` field of the OAuth2 success response ([#6490](https://github.com/alvarolm/pocketbase/issues/6490)).
...
- Added a temporary exception for Backblaze S3 endpoints to exclude the new `aws-sdk-go-v2` checksum headers ([#6440](https://github.com/alvarolm/pocketbase/discussions/6440)).


## v0.25.2

- Fixed realtime delete event not being fired for `RecordProxy`-ies and added basic realtime record resolve automated tests ([#6433](https://github.com/alvarolm/pocketbase/issues/6433)).
...
- Added Trakt OAuth2 provider ([#6338](https://github.com/alvarolm/pocketbase/pull/6338); thanks @aidan-)

- Added support for case-insensitive password auth based on the related UNIQUE index field collation ([#6337](https://github.com/alvarolm/pocketbase/discussions/6337)).

- Enforced `when_required` for the new AWS SDK request and response checksum validations to allow other non-AWS vendors to catch up with new AWS SDK changes (see [#6313](https://github.com/alvarolm/pocketbase/discussions/6313) and [aws/aws-sdk-go-v2#2960](https://github.com/aws/aws-sdk-go-v2/discussions/2960)).
...
- Soft-deprecated `Record.GetUploadedFiles` in favor of `Record.GetUnsavedFiles` to minimize the ambiguities what the method do ([#6269](https://github.com/alvarolm/pocketbase/discussions/6269)).
...
- Added JSVM `new Timezone(name)` binding for constructing `time.Location` value ([#6219](https://github.com/alvarolm/pocketbase/discussions/6219)).
...
- Fixed fields extraction for view query with nested comments ([#6309](https://github.com/alvarolm/pocketbase/discussions/6309)).
...
- Fixed incorrectly reported unique validator error for fields starting with name of another field ([#6281](https://github.com/alvarolm/pocketbase/pull/6281); thanks @svobol13).
...
- Fixed display fields extraction when there are multiple "Presentable" `relation` fields in a single related collection ([#6229](https://github.com/alvarolm/pocketbase/issues/6229)).
...
- ⚠️ Removed the "dry submit" when executing the collections Create API rule
    (you can find more details why this change was introduced and how it could affect your app in https://github.com/alvarolm/pocketbase/discussions/6073).
...
- Added auth collection select for the settings "Send test email" popup ([#6166](https://github.com/alvarolm/pocketbase/issues/6166)).
...
- Invalidate all record tokens when the auth record email is changed programmatically or by a superuser ([#5964](https://github.com/alvarolm/pocketbase/issues/5964)).
...
- Fixed "Unique identify fields" input not refreshing on unique indexes change ([#6184](https://github.com/alvarolm/pocketbase/issues/6184)).


## v0.23.12

- Added warning logs in case of mismatched `modernc.org/sqlite` and `modernc.org/libc` versions ([#6136](https://github.com/alvarolm/pocketbase/issues/6136#issuecomment-2556336962)).

- Skipped the default body size limit middleware for the backup upload endpoint ([#6152](https://github.com/alvarolm/pocketbase/issues/6152)).
...
- Renew the superuser file token cache when clicking on the thumb preview or download link ([#6137](https://github.com/alvarolm/pocketbase/discussions/6137)).
...
- Replaced `strconv.Itoa` with `strconv.FormatInt` to avoid the int64->int conversion overflow on 32-bit platforms ([#6132](https://github.com/alvarolm/pocketbase/discussions/6132)).


## v0.23.8

- Fixed Model->Record and Model->Collection hook events sync for nested and/or inner-hook transactions ([#6122](https://github.com/alvarolm/pocketbase/discussions/6122)).
...
- Fixed JSVM exception -> Go error unwrapping when throwing errors from non-request hooks ([#6102](https://github.com/alvarolm/pocketbase/discussions/6102)).


## v0.23.6

- Fixed `$filesystem.fileFromURL` documentation and generated type ([#6058](https://github.com/alvarolm/pocketbase/issues/6058)).

- Fixed `X-Forwarded-For` header typo in the suggested UI "Common trusted proxy" headers ([#6063](https://github.com/alvarolm/pocketbase/pull/6063)).

- Updated the `text` field max length validator error message to make it more clear ([#6066](https://github.com/alvarolm/pocketbase/issues/6066)).
...
- Fixed `text` field max validation error message ([#6053](https://github.com/alvarolm/pocketbase/issues/6053)).
...
- Fixed `autodate` fields not refreshing when calling `Save` multiple times on the same `Record` instance ([#6000](https://github.com/alvarolm/pocketbase/issues/6000)).

- Added more descriptive test OTP id and failure log message ([#5982](https://github.com/alvarolm/pocketbase/discussions/5982)).

- Moved the default UI CSP from meta tag to response header ([#5995](https://github.com/alvarolm/pocketbase/discussions/5995)).
...
- Fixed `Record.Fresh()`/`Record.Clone()` methods not properly cloning `autodate` fields ([#5973](https://github.com/alvarolm/pocketbase/discussions/5973)).


## v0.23.2

- Fixed `RecordQuery()` custom struct scanning ([#5958](https://github.com/alvarolm/pocketbase/discussions/5958)).
...
- Added `RequestEvent.Blob(status, contentType, bytes)` response write helper ([#5940](https://github.com/alvarolm/pocketbase/discussions/5940)).
...
- Linear OAuth2 provider ([#5909](https://github.com/alvarolm/pocketbase/pull/5909); thanks @chnfyi).
- WakaTime OAuth2 provider ([#5829](https://github.com/alvarolm/pocketbase/pull/5829); thanks @tigawanna).
- Notion OAuth2 provider ([#4999](https://github.com/alvarolm/pocketbase/pull/4999); thanks @s-li1).
- monday.com OAuth2 provider ([#5346](https://github.com/alvarolm/pocketbase/pull/5346); thanks @Jaytpa01).
- New Instagram provider compatible with the new Instagram Login APIs ([#5588](https://github.com/alvarolm/pocketbase/pull/5588); thanks @pnmcosta).
...
- ⚠️ Changes to the app settings model fields and response (+new options such as `trustedProxy`, `rateLimits`, `batch`, etc.). The app settings Web APIs are mostly used by the Dashboard UI and rarely by the end users, but if you want to check all settings changes please refer to the [Settings Go struct](https://github.com/alvarolm/pocketbase/blob/develop/core/settings_model.go#L121).

- ⚠️ New flatten Collection model and fields structure. The Collection model Web APIs are mostly used by the Dashboard UI and rarely by the end users, but if you want to check all changes please refer to the [Collection Go struct](https://github.com/alvarolm/pocketbase/blob/develop/core/collection_model.go#L308).
