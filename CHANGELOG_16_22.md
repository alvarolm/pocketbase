...
- (_Backported from v0.26.6_) Allow OIDC `email_verified` to be int or boolean string since some OIDC providers like AWS Cognito has non-standard userinfo response ([#6657](https://github.com/alvarolm/pocketbase/pull/6657)).


## v0.22.33

- (_Backported from v0.26.3_) Fixed and normalized logs error serialization across common types for more consistent logs error output ([#6631](https://github.com/alvarolm/pocketbase/issues/6631)).
...
- (_Backported from v0.24.4_) Fixed fields extraction for view queries with nested comments ([#6309](https://github.com/alvarolm/pocketbase/discussions/6309)).
...
- (_Backported from v0.23.10_) Renew the superuser file token cache when clicking on the thumb preview or download link ([#6137](https://github.com/alvarolm/pocketbase/discussions/6137)).
...
- Instead of unregistering the realtime clients, we now just unset their auth state on delete of the related auth record so that the clients can receive the `delete` event ([#5898](https://github.com/alvarolm/pocketbase/issues/5898)).
...
- Refresh the old collections state in the Import UI after successful import submission ([#5861](https://github.com/alvarolm/pocketbase/issues/5861)).
...
- Delete new uploaded record files in case of DB persist error ([#5845](https://github.com/alvarolm/pocketbase/issues/5845)).


## v0.22.23

- Updated the hooks watcher to account for the case when hooksDir is a symlink ([#5789](https://github.com/alvarolm/pocketbase/issues/5789)).
...
- Added deprecation log in case Instagram OAuth2 is used (_related to [#5652](https://github.com/alvarolm/pocketbase/discussions/5652)_).
...
- Lock the logs database during backup to prevent `database disk image is malformed` errors in case there is a log write running in the background ([#5541](https://github.com/alvarolm/pocketbase/discussions/5541)).


## v0.22.20

- Fixed the Admin UI `isEmpty` check to allow submitting zero uuid, datetime and date strings ([#5398](https://github.com/alvarolm/pocketbase/issues/5398)).
...
- Added additional parsing for the Apple OAuth2 `user` token response field to attempt returning the name of the authenticated user ([#5074](https://github.com/alvarolm/pocketbase/discussions/5074#discussioncomment-10317207)).
...
- Improved files delete performance when using the local filesystem by adding a trailing slash to the `DeletePrefix` call to ensure that the list iterator will start "walking" from the prefix directory and not from its parent ([#5246](https://github.com/alvarolm/pocketbase/discussions/5246)).
...
- Fixed the days calculation for triggering old logs deletion ([#5179](https://github.com/alvarolm/pocketbase/pull/5179); thanks @nehmeroumani).
...
- Added mutex to `tests.TestMailer()` to minimize tests data race warnings ([#5157](https://github.com/alvarolm/pocketbase/issues/5157)).
...
- Added OAuth2 POST redirect support (in case of `response_mode=form_post`) to allow specifying scopes for the Apple OAuth2 integration.
    ...
    Note 2: Existing users that have already logged in with Apple may need to revoke their access in order to see the email sharing options as shown in [this screenshot](https://github.com/alvarolm/pocketbase/discussions/5074#discussioncomment-9801855).
...
- ⚠️ Fixed a security vulnerability related to the OAuth2 email autolinking (thanks to @dalurness for reporting it).
    ...
    Just to be safe I've also published a [GitHub security advisory](https://github.com/alvarolm/pocketbase/security/advisories/GHSA-m93w-4fxv-r35v) (_may take some time to show up in the related security databases_).
    ...
    **If you don't recognize the above action, you should immediately change your Acme account password.**
    ...
    The flow will be further improved with the [ongoing refactoring](https://github.com/alvarolm/pocketbase/discussions/4355) and we will start sending emails for "unrecognized device" logins (OTP and MFA is already implemented and will be available with the next v0.23.0 release in the near future).


## v0.22.13

- Fixed rules inconsistency for text literals when inside parenthesis ([#5017](https://github.com/alvarolm/pocketbase/issues/5017)).
...
- Fixed calendar picker grid layout misalignment on Firefox ([#4865](https://github.com/alvarolm/pocketbase/issues/4865)).
...
- Load the full record in the relation picker edit panel ([#4857](https://github.com/alvarolm/pocketbase/issues/4857)).


## v0.22.10

- Updated the uploaded filename normalization to take double extensions in consideration ([#4824](https://github.com/alvarolm/pocketbase/issues/4824))

- Added Collection models cache to help speed up the common List and View requests execution with ~25%.
  _This was extracted from the ongoing work on [#4355](https://github.com/alvarolm/pocketbase/discussions/4355) and there are many other small optimizations already implemented but they will have to wait for the refactoring to be finalized._


## v0.22.9

- Fixed Admin UI OAuth2 "Clear all fields" btn action to properly unset all form fields ([#4737](https://github.com/alvarolm/pocketbase/issues/4737)).


## v0.22.8

- Fixed '~' auto wildcard wrapping when the param has escaped `%` character ([#4704](https://github.com/alvarolm/pocketbase/discussions/4704)).
...
- Other minor improvements (updated GitLab OAuth2 provider logo [#4650](https://github.com/alvarolm/pocketbase/pull/4650), normalized error messages, updated npm dependencies, etc.)


## v0.22.6

- Admin UI accessibility improvements:
  - Fixed the dropdowns tab/enter/space keyboard navigation ([#4607](https://github.com/alvarolm/pocketbase/issues/4607)).
  - Added `role`, `aria-label`, `aria-hidden` attributes to some of the elements in attempt to better assist screen readers.


## v0.22.5

- Minor test helpers fixes ([#4600](https://github.com/alvarolm/pocketbase/issues/4600)):
...
- Removed conflicting styles causing the detailed codeblock log data preview to not visualize properly ([#4505](https://github.com/alvarolm/pocketbase/pull/4505)).

- Minor JSVM improvements:
  - Added `$filesystem.fileFromUrl(url, optSecTimeout)` helper.
  - Implemented the `FormData` interface and added support for sending `multipart/form-data` requests with `$http.send()` ([#4544](https://github.com/alvarolm/pocketbase/discussions/4544)).


## v0.22.3

- Fixed the z-index of the current admin dropdown on Safari ([#4492](https://github.com/alvarolm/pocketbase/issues/4492)).

- Fixed `OnAfterApiError` debug log `nil` error reference ([#4498](https://github.com/alvarolm/pocketbase/issues/4498)).

- Added the field name as part of the `@request.data.someRelField.*` join to handle the case when a collection has 2 or more relation fields pointing to the same place ([#4500](https://github.com/alvarolm/pocketbase/issues/4500)).
...
- Fixed Admin UI record and collection panels not reinitializing properly on browser back/forward navigation ([#4462](https://github.com/alvarolm/pocketbase/issues/4462)).

- Initialize `RecordAuthWithOAuth2Event.IsNewRecord` for the `OnRecordBeforeAuthWithOAuth2Request` hook ([#4437](https://github.com/alvarolm/pocketbase/discussions/4437)).

- Added error checks to the autogenerated Go migrations ([#4448](https://github.com/alvarolm/pocketbase/issues/4448)).


## v0.22.0

- Added Planning Center OAuth2 provider ([#4393](https://github.com/alvarolm/pocketbase/pull/4393); thanks @alxjsn).
...
- Admin UI improvements:
  ...
  - Added option to import a subset of collections without deleting the others ([#3403](https://github.com/alvarolm/pocketbase/issues/3403)).
...
- Adjusted the `cron.Start()` to start the ticker at the `00` second of the cron interval ([#4394](https://github.com/alvarolm/pocketbase/discussions/4394)).
...
- Fixed auto backups cron not reloading properly after app settings change ([#4431](https://github.com/alvarolm/pocketbase/discussions/4431)).

- Upgraded to `aws-sdk-go-v2` and added special handling for GCS to workaround the previous [GCS headers signature issue](https://github.com/alvarolm/pocketbase/issues/2231) that we had with v2.
  _This should also fix the SVG/JSON zero response when using Cloudflare R2 ([#4287](https://github.com/alvarolm/pocketbase/issues/4287#issuecomment-1925168142), [#2068](https://github.com/alvarolm/pocketbase/discussions/2068), [#2952](https://github.com/alvarolm/pocketbase/discussions/2952))._
...
- Ignore the JS required validations for disabled OIDC providers ([#4322](https://github.com/alvarolm/pocketbase/issues/4322)).

- Allow `HEAD` requests to the `/api/health` endpoint ([#4310](https://github.com/alvarolm/pocketbase/issues/4310)).
...
- Fixed `@request.auth.*` initialization side-effect which caused the current authenticated user email to not being returned in the user auth response ([#2173](https://github.com/alvarolm/pocketbase/issues/2173#issuecomment-1932332038)).
...
- Added Bitbucket OAuth2 provider ([#3948](https://github.com/alvarolm/pocketbase/pull/3948); thanks @aabajyan).

- Mark user as verified on confirm password reset ([#4066](https://github.com/alvarolm/pocketbase/issues/4066)).
...
- Added graceful OAuth2 redirect error handling ([#4177](https://github.com/alvarolm/pocketbase/issues/4177)).
...
- Fixed the Admin UI auto indexes update when renaming fields with a common prefix ([#4160](https://github.com/alvarolm/pocketbase/issues/4160)).


## v0.20.6

- Fixed JSVM types generation for functions with omitted arg types ([#4145](https://github.com/alvarolm/pocketbase/issues/4145)).
...
- Minor CSS fix for the Admin UI to prevent the searchbar within a popup from expanding too much and pushing the controls out of the visible area ([#4079](https://github.com/alvarolm/pocketbase/issues/4079#issuecomment-1876994116)).


## v0.20.4

- Small fix for a regression introduced with the recent `json` field changes that was causing View collection column expressions recognized as `json` to fail to resolve ([#4072](https://github.com/alvarolm/pocketbase/issues/4072)).


## v0.20.3

- Fixed the `json` field query comparisons to work correctly with plain JSON values like `null`, `bool` `number`, etc. ([#4068](https://github.com/alvarolm/pocketbase/issues/4068)).
...
- Fixed multi-line text paste in the Admin UI search bar ([#4022](https://github.com/alvarolm/pocketbase/discussions/4022)).
...
- Added `--dev` flag and its accompanying `app.IsDev()` method (_in place of the previously removed `--debug`_) to assist during development ([#3918](https://github.com/alvarolm/pocketbase/discussions/3918)).
...
- Replaced the default API tests timeout with a new `ApiScenario.Timeout` option ([#3930](https://github.com/alvarolm/pocketbase/issues/3930)).
...
- OAuth2 related additions:
    ...
    - Added new `PKCE()` and `SetPKCE(enable)` OAuth2 methods to indicate whether the PKCE flow is supported or not.
      ...
      _This was added to accommodate OIDC providers that may throw an error if unsupported PKCE params are submitted with the auth request (eg. LinkedIn; see [#3799](https://github.com/alvarolm/pocketbase/discussions/3799#discussioncomment-7640312))._
    ...
    - Added `expiry` field to the OAuth2 user response containing the _optional_ expiration time of the OAuth2 access token ([#3617](https://github.com/alvarolm/pocketbase/discussions/3617)).
...
- Minor JSVM updates and fixes:
    ...
    - Added `$apis.requireGuestOnly()` middleware JSVM binding ([#3896](https://github.com/alvarolm/pocketbase/issues/3896)).
...
- Added `MaxSize` `json` field option to prevent storing large json data in the db ([#3790](https://github.com/alvarolm/pocketbase/issues/3790)).
...
- Limit the concurrent thumbs generation to avoid high CPU and memory usage in spiky scenarios ([#3794](https://github.com/alvarolm/pocketbase/pull/3794); thanks @t-muehlberger).
...
- Fixed TinyMCE source code viewer textarea styles ([#3715](https://github.com/alvarolm/pocketbase/issues/3715)).

- Fixed `text` field min/max validators to properly count multi-byte characters ([#3735](https://github.com/alvarolm/pocketbase/issues/3735)).

- Allowed hyphens in `username` ([#3697](https://github.com/alvarolm/pocketbase/issues/3697)).
...
- Added the release notes to the console output of `./pocketbase update` ([#3685](https://github.com/alvarolm/pocketbase/discussions/3685)).

- Added missing documentation for the JSVM `$mails.*` bindings.

- Relaxed the OAuth2 redirect url validation to allow any string value ([#3689](https://github.com/alvarolm/pocketbase/pull/3689); thanks @sergeypdev).
  _Note that the redirect url format is still bound to the accepted values by the specific OAuth2 provider._


## v0.19.2

- Updated the JSVM generated types ([#3627](https://github.com/alvarolm/pocketbase/issues/3627), [#3662](https://github.com/alvarolm/pocketbase/issues/3662)).
...
- Fixed `tokenizer.Scan()/ScanAll()` to ignore the separators from the default trim cutset.
  ...
  _This should fix the parsing of whitespace characters around view query column names when no quotes are used ([#3616](https://github.com/alvarolm/pocketbase/discussions/3616#discussioncomment-7398564))._
...
- Added Patreon OAuth2 provider ([#3323](https://github.com/alvarolm/pocketbase/pull/3323); thanks @ghostdevv).

- Added mailcow OAuth2 provider ([#3364](https://github.com/alvarolm/pocketbase/pull/3364); thanks @thisni1s).
...
- Several Admin UI improvements:
  - Count the total records separately to speed up the query execution for large datasets ([#3344](https://github.com/alvarolm/pocketbase/issues/3344)).
  - Enclosed the listing scrolling area within the table so that the horizontal scrollbar and table header are always reachable ([#2505](https://github.com/alvarolm/pocketbase/issues/2505)).
  - Allowed opening the record preview/update form via direct URL ([#2682](https://github.com/alvarolm/pocketbase/discussions/2682)).
...
- Removed the explicit `Content-Type` charset from the realtime response due to compatibility issues with IIS ([#3461](https://github.com/alvarolm/pocketbase/issues/3461)).
...
- Added global `raw` template function to allow outputting raw/verbatim HTML content in the JSVM templates ([#3476](https://github.com/alvarolm/pocketbase/discussions/3476)).
  ```
  {{.description|raw}}
  ```

- Trimmed view query semicolon and allowed single quotes for column aliases ([#3450](https://github.com/alvarolm/pocketbase/issues/3450#issuecomment-1748044641)).
...
- Fixed empty thumbs directories not getting deleted on Windows after deleting a record img file ([#3382](https://github.com/alvarolm/pocketbase/issues/3382)).
...
- Fixed JS error in the Admin UI when listing records with invalid `relation` field value ([#3372](https://github.com/alvarolm/pocketbase/issues/3372)).
...
- Return the response headers and cookies in the `$http.send()` result ([#3310](https://github.com/alvarolm/pocketbase/discussions/3310)).
...
- Added escape character (`\`) support in the Admin UI to allow using `select` field values with comma ([#2197](https://github.com/alvarolm/pocketbase/discussions/2197)).


## v0.18.3

- Exposed a global JSVM `readerToString(reader)` helper function to allow reading Go `io.Reader` values ([#3273](https://github.com/alvarolm/pocketbase/discussions/3273)).
...
- Prevent breaking the record form in the Admin UI in case the browser's localStorage quota has been exceeded when uploading or storing large `editor` values ([#3265](https://github.com/alvarolm/pocketbase/issues/3265)).

- Updated docs and missing JSVM typings.

- Exposed additional crypto primitives under the `$security.*` JSVM namespace ([#3273](https://github.com/alvarolm/pocketbase/discussions/3273)):
  ```js
  // HMAC with SHA256
...
- Excluded the local temp dir from the backups ([#3261](https://github.com/alvarolm/pocketbase/issues/3261)).


## v0.18.0

- Simplified the `serve` command to accept domain name(s) as argument to reduce any additional manual hosts setup that sometimes previously was needed when deploying on production ([#3190](https://github.com/alvarolm/pocketbase/discussions/3190)).
  ```sh
...
- Added option to upload a backup file from the Admin UI ([#2599](https://github.com/alvarolm/pocketbase/issues/2599)).
...
- Added the application name as part of the autogenerated backup name for easier identification ([#3066](https://github.com/alvarolm/pocketbase/issues/3066)).

- Added new `SmtpConfig.LocalName` option to specify a custom domain name (or IP address) for the initial EHLO/HELO exchange ([#3097](https://github.com/alvarolm/pocketbase/discussions/3097)).
...
- `editor` field improvements:
    ...
    - Normalized pasted text while still preserving links, lists, tables, etc. formatting ([#3257](https://github.com/alvarolm/pocketbase/issues/3257)).

- Added option to auto generate admin and auth record passwords from the Admin UI.

- Added JSON validation and syntax highlight for the `json` field in the Admin UI ([#3191](https://github.com/alvarolm/pocketbase/issues/3191)).
...
- Added cron expression macros ([#3132](https://github.com/alvarolm/pocketbase/issues/3132)):
  ```
  @yearly   - "0 0 1 1 *"
  @annually - "0 0 1 1 *"
  @monthly  - "0 0 1 * *"
...
- Added JSVM `$mails.*` binds for the corresponding Go [mails package](https://pkg.go.dev/github.com/alvarolm/pocketbase/mails) functions.
...
- ⚠️ Fill the `LastVerificationSentAt` and `LastResetSentAt` fields only after a successfull email send ([#3121](https://github.com/alvarolm/pocketbase/issues/3121)).

- ⚠️ Skip API `fields` json transformations for non 20x responses ([#3176](https://github.com/alvarolm/pocketbase/issues/3176)).
...
- ⚠️ Changes to `tests.ApiScenario` struct:
    ...
    - The `ApiScenario.TestAppFactory` now accept the test instance as argument and no longer expect an error as return result ([#3025](https://github.com/alvarolm/pocketbase/discussions/3025#discussioncomment-6592272)).
      ```go
      // old
...
- Fixed the autogenerated `down` migrations to properly revert the old collection rules in case a change was made in `up` ([#3192](https://github.com/alvarolm/pocketbase/pull/3192); thanks @impact-merlinmarek).
...
- Fixed JSVM `require()` file path error when using Windows-style path delimiters ([#3163](https://github.com/alvarolm/pocketbase/issues/3163#issuecomment-1685034438)).
...
- Fixed Views record retrieval when numeric id is used ([#3110](https://github.com/alvarolm/pocketbase/issues/3110)).
  _With this fix we also now properly recognize `CAST(... as TEXT)` and `CAST(... as BOOLEAN)` as `text` and `bool` fields._

- Fixed `relation` "Cascade delete" tooltip message ([#3098](https://github.com/alvarolm/pocketbase/issues/3098)).

- Fixed jsvm error message prefix on failed migrations ([#3103](https://github.com/alvarolm/pocketbase/pull/3103); thanks @nzhenev).

- Disabled the initial Admin UI admins counter cache when there are no initial admins to allow detecting externally created accounts (eg. with the `admin` command) ([#3106](https://github.com/alvarolm/pocketbase/issues/3106)).
...
- Fixed Docker `cross-device link` error when creating `pb_data` backups on a local mounted volume ([#3089](https://github.com/alvarolm/pocketbase/issues/3089)).

- Fixed the error messages for relation to views ([#3090](https://github.com/alvarolm/pocketbase/issues/3090)).
...
- Soft-deprecated `$http.send({ data: object, ... })` in favour of `$http.send({ body: rawString, ... })`
  to allow sending non-JSON body with the request ([#3058](https://github.com/alvarolm/pocketbase/discussions/3058)).
...
- Added `core.RealtimeConnectEvent.IdleTimeout` field to allow specifying a different realtime idle timeout duration per client basis ([#3054](https://github.com/alvarolm/pocketbase/discussions/3054)).

- Fixed `apis.RequestData` deprecation log note ([#3068](https://github.com/alvarolm/pocketbase/pull/3068); thanks @gungjodi).


## v0.17.1

- Use relative path when redirecting to the OAuth2 providers page in the Admin UI to support subpath deployments ([#3026](https://github.com/alvarolm/pocketbase/pull/3026); thanks @sonyarianto).

- Manually trigger the `OnBeforeServe` hook for `tests.ApiScenario` ([#3025](https://github.com/alvarolm/pocketbase/discussions/3025)).

- Trigger the JSVM `cronAdd()` handler only on app `serve` to prevent unexpected (and eventually duplicated) cron handler calls when custom console commands are used ([#3024](https://github.com/alvarolm/pocketbase/discussions/3024#discussioncomment-6592703)).
...
- Added Instagram OAuth2 provider ([#2534](https://github.com/alvarolm/pocketbase/pull/2534); thanks @pnmcosta).

- Added VK OAuth2 provider ([#2533](https://github.com/alvarolm/pocketbase/pull/2533); thanks @imperatrona).

- Added Yandex OAuth2 provider ([#2762](https://github.com/alvarolm/pocketbase/pull/2762); thanks @imperatrona).
...
- Added new utility `github.com/alvarolm/pocketbase/tools/template` subpackage to assist with rendering HTML templates using the standard Go `html/template` and `text/template` syntax.
...
- ⚠️ Changes to the List/Search APIs
    - Added new query parameter `?skipTotal=1` to skip the `COUNT` query performed with the list/search actions ([#2965](https://github.com/alvarolm/pocketbase/discussions/2965)).
...
- ⚠️ Disallowed relations to views **from non-view** collections ([#3000](https://github.com/alvarolm/pocketbase/issues/3000)).
...
- Allowed `0` as `RelationOptions.MinSelect` value to avoid the ambiguity between 0 and non-filled input value ([#2817](https://github.com/alvarolm/pocketbase/discussions/2817)).

- Fixed zero-default value not being used if the field is not explicitly set when manually creating records ([#2992](https://github.com/alvarolm/pocketbase/issues/2992)).
...
- Added multiple valued fields (`relation`, `select`, `file`) normalizations to ensure that the zero-default value of a newly created multiple field is applied for already existing data ([#2930](https://github.com/alvarolm/pocketbase/issues/2930)).


## v0.16.9

- Register the `eagerRequestInfoCache` middleware only for the internal `api` group routes to avoid conflicts with custom route handlers ([#2914](https://github.com/alvarolm/pocketbase/issues/2914)).


## v0.16.8

- Fixed unique validator detailed error message not being returned when camelCase field name is used ([#2868](https://github.com/alvarolm/pocketbase/issues/2868)).

- Updated the index parser to allow no space between the table name and the columns list ([#2864](https://github.com/alvarolm/pocketbase/discussions/2864#discussioncomment-6373736)).
...
- Fixed collection index column sort normalization in the Admin UI ([#2681](https://github.com/alvarolm/pocketbase/pull/2681); thanks @SimonLoir).

- Removed unnecessary admins count in `apis.RequireAdminAuthOnlyIfAny()` middleware ([#2726](https://github.com/alvarolm/pocketbase/pull/2726); thanks @svekko).

- Fixed `multipart/form-data` request bind not populating map array values ([#2763](https://github.com/alvarolm/pocketbase/discussions/2763#discussioncomment-6278902)).
...
- Fixed the Admin UI serialization of implicit relation display fields ([#2675](https://github.com/alvarolm/pocketbase/issues/2675)).
...
- Fixed the selfupdate command not working on Windows due to missing `.exe` in the extracted binary path ([#2589](https://github.com/alvarolm/pocketbase/discussions/2589)).
...
- Added `int64`, `int32`, `uint`, `uint64` and `uint32` support when scanning `types.DateTime` ([#2602](https://github.com/alvarolm/pocketbase/discussions/2602))
...
- Fixed schema fields sort not working on Safari/Gnome Web ([#2567](https://github.com/alvarolm/pocketbase/issues/2567)).

- Fixed default `PRAGMA`s not being applied for new connections ([#2570](https://github.com/alvarolm/pocketbase/discussions/2570)).


## v0.16.2

- Fixed backups archive not excluding the local `backups` directory on Windows ([#2548](https://github.com/alvarolm/pocketbase/discussions/2548#discussioncomment-5979712)).

- Changed file field to not use `dataTransfer.effectAllowed` when dropping files since it is not reliable and consistent across different OS and browsers ([#2541](https://github.com/alvarolm/pocketbase/issues/2541)).

- Auto register the initial generated snapshot migration to prevent incorrectly reapplying the snapshot on Docker restart ([#2551](https://github.com/alvarolm/pocketbase/discussions/2551)).
...
- Fixed backup restore not working in a container environment when `pb_data` is mounted as volume ([#2519](https://github.com/alvarolm/pocketbase/issues/2519)).

- Fixed Dart SDK realtime API preview example ([#2523](https://github.com/alvarolm/pocketbase/pull/2523); thanks @xFrann).

- Fixed typo in the backups create panel ([#2526](https://github.com/alvarolm/pocketbase/pull/2526); thanks @dschissler).

- Removed unnecessary slice length check in `list.ExistInSlice` ([#2527](https://github.com/alvarolm/pocketbase/pull/2527); thanks @KunalSin9h).

- Avoid mutating the cached request data on OAuth2 user create ([#2535](https://github.com/alvarolm/pocketbase/discussions/2535)).

- Fixed Export Collections "Download as JSON" ([#2540](https://github.com/alvarolm/pocketbase/issues/2540)).

- Fixed file field drag and drop not working in Firefox and Safari ([#2541](https://github.com/alvarolm/pocketbase/issues/2541)).
...
- Added `accept` file field attribute with the field MIME types ([#2466](https://github.com/alvarolm/pocketbase/pull/2466); thanks @Nikhil1920).

- Added support for multiple files sort in the Admin UI ([#2445](https://github.com/alvarolm/pocketbase/issues/2445)).
